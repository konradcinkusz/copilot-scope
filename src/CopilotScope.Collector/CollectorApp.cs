...
namespace CopilotScope.Collector;

/// <summary>
/// The collector as a buildable application rather than a program: everything <c>Program.cs</c>
/// used to do inline, so the same collector can run on its own (<c>Program.cs</c>, the
/// container images, the Aspire AppHost) or inside another process — the native
/// <c>copilotscope</c> host of ADR-004, which runs it beside the dashboard. Nothing here knows
/// which of the two it is in.
/// </summary>
public static class CollectorApp
{
    /// <summary>The collector's own <c>appsettings.json</c>, compiled in (see
    /// <see cref="ServiceDefaults.Extensions.AddEmbeddedDefaults{TBuilder}"/>).</summary>
    internal const string DefaultsResource = "CopilotScope.Collector.appsettings.json";

    /// <summary>
    /// Builds the collector: services, the OTLP and query endpoints, the background loops.
    /// The caller runs it.
    /// </summary>
    /// <param name="options">Passed to <see cref="WebApplication.CreateBuilder(WebApplicationOptions)"/>.
    /// A host that embeds the collector sets the content root, environment and application name
    /// here: none of them can be changed once the builder exists.</param>
    /// <param name="configure">Runs immediately after the builder is created and before anything
    /// reads configuration, so a host can add configuration sources or set Kestrel endpoints
    /// that the rest of the build will honour.</param>
    public static async Task<WebApplication> BuildAsync(WebApplicationOptions options,
        Action<WebApplicationBuilder>? configure = null)
    {
        var builder = WebApplication.CreateBuilder(options);
        // Defaults such as the model pricing table live in appsettings.json. A host whose
        // content root has none — the native binary publishes no copy of it — still gets them.
        builder.AddEmbeddedDefaults(typeof(CollectorApp).Assembly, DefaultsResource);
